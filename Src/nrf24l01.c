...
#define NRF24L01_FLUSH_TX	do { NRF24L01_CSN_LOW; HAL_SPI_Transmit(&hspi1, &tmpx, 1, SPI_WriteTimeout); NRF24L01_CSN_HIGH; } while (0)
#define NRF24L01_FLUSH_RX	do { NRF24L01_CSN_LOW; HAL_SPI_Transmit(&hspi1, &tmpx1, 1, SPI_WriteTimeout); NRF24L01_CSN_HIGH; } while (0)
...
uint8_t TM_NRF24L01_ReadRegister(uint8_t reg) {
  ...
  HAL_SPI_Transmit (&hspi1, &tmp, 1, SPI_WriteTimeout);
  tmp = NRF24L01_NOP_MASK;
  HAL_SPI_TransmitReceive(&hspi1, &tmp, &value, 1, SPI_WriteTimeout);
  NRF24L01_CSN_HIGH;
...
void TM_NRF24L01_ReadRegisterMulti(uint8_t reg, uint8_t* data, uint8_t count) {
  ...
  HAL_SPI_Transmit (&hspi1, &tmp1, 1, SPI_WriteTimeout);
  
  uint8_t tmp2 = NRF24L01_NOP_MASK;
  //this might be needed to change into HAL_SPI_TransmitReceive
  HAL_SPI_Transmit(&hspi1, &tmp2, 1, SPI_WriteTimeout); 
  HAL_SPI_Receive(&hspi1, data, count, SPI_ReadTimeout);
...
void TM_NRF24L01_WriteRegister(uint8_t reg, uint8_t value) {
  ...
  HAL_SPI_Transmit (&hspi1, &tmp, 1, SPI_WriteTimeout);
  HAL_SPI_Transmit (&hspi1, &value, 1, SPI_WriteTimeout);
...
void TM_NRF24L01_WriteRegisterMulti(uint8_t reg, uint8_t *data, uint8_t count) {
  ...
  uint8_t tmp = NRF24L01_WRITE_REGISTER_MASK(reg);
  HAL_SPI_Transmit(&hspi1, &tmp, 1, SPI_WriteTimeout);     
  HAL_SPI_Transmit(&hspi1, data, count, SPI_WriteTimeout);
  NRF24L01_CSN_HIGH;
}
...
void TM_NRF24L01_GetData(uint8_t* data) {
  ...
  HAL_SPI_Transmit(&hspi1, &tmp, 1, SPI_WriteTimeout);
  /* Read payload */
  HAL_SPI_TransmitReceive(&hspi1, data, data, TM_NRF24L01_Struct.PayloadSize, SPI_ReadTimeout);
  /* Pull up chip select */
  NRF24L01_CSN_HIGH;
...
uint8_t TM_NRF24L01_GetStatus(void) {
  ...
  HAL_SPI_TransmitReceive(&hspi1, &tmp, &status, 1, SPI_WriteTimeout);
  /* Pull up chip select */
  NRF24L01_CSN_HIGH;
